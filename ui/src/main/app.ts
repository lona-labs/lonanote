...
import { initInvokeIpc } from './invoke';
...
const getTitleBarOverlay = (zoom: number): TitleBarOverlay | undefined => {
  ...
  const height = settings.getZoomActualPx(zoom, settings.defaultTitleBarHeight);
...
export const setupApp = async () => {
  ...
  app.whenReady().then(() => {
    ...
    ipcMain.on('setTitleBarColor', (e, color, backgroudColor) => {
      if (mainWindow && color) {
